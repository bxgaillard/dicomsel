// ImagePosition.h
#ifndef DICOM_IMAGEPOSITION_TAG_H
#define DICOM_IMAGEPOSITION_TAG_H
...
class CImagePosition : public CDicomTag
{
private:
    std::vector<double> m_vImagePosition;
...
public :
	 ...
	 *********************
	 * @pre Group == 0x0020 && Element 0x0032
	 *********************
	 * @see dicom::tag::CDicomTag
	 */
    explicit				CImagePosition							( const int32 _i32Group, const int32 _i32Element, const int32 _i32Length );

    const std::vector<double> GetPosition							( void ) const ;

    const double			GetPositionX							( void ) const ;

    const double			GetPositionY							( void ) const ;

    const double			GetPositionZ							( void ) const ;
...
#endif // DICOM_IMAGEPOSITION_TAG_H
