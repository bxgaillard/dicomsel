...
# Modified by:
# Created:     8/9/2004
# RCS-ID:      $Id: wx-config.in,v 1.119 2005/02/13 17:00:48 SN Exp $
...
prefix=${input_option_prefix-${this_prefix:-/tmp/xbuildwx/build}}
...
is_cross && target="i386-mingw32msvc"
...
[ -z "$output_option_release"      ] || echo "2.6"
[ -z "$output_option_version"      ] || echo "2.6.1"
[ -z "$output_option_version_full" ] || echo "2.6.1.0"
[ -z "$output_option_basename"     ] || echo "wx_msw"
[ -z "$output_option_rezflags"     ] || echo $(eval echo "@true")
[ -z "$output_option_cc"           ] || echo "i386-mingw32msvc-gcc"
[ -z "$output_option_cxx"          ] || echo "i386-mingw32msvc-c++"
[ -z "$output_option_ld"           ] || echo "i386-mingw32msvc-c++ -o"
...
if [ -n "$input_option_utility" ]; then
    ...
    for _util in "${input_option_utility}-2.6" \
                 "${input_option_utility}-2.6"             \
...
ldlibs_base="-lrpcrt4 -loleaut32 -lole32 -luuid -lwinspool -lwinmm -lshell32 -lcomctl32 -lcomdlg32 -lctl3d32 -ladvapi32 -lwsock32 -lgdi32"
ldlibs_core=""
ldlibs_gl=""
ldlibs_html=""
ldlibs_xml=""
...
lib_flags_for()
{
    ...
    for lib; do
        ...
        if match_field "$lib" net base ; then
            _libname="wx_base"
        else
            _libname="wx_msw"
        fi
        [ $lib = base ] || _libname="${_libname}_$lib"
        _libname="${_libname}-2.6$_target"
...
if is_monolithic; then
    ...
    for i in std adv core net base; do
        wx_libs=$(remove_field $i $wx_libs)
    done

    wx_libs=" -Wl,--subsystem,windows -mwindows $(lib_flags_for $wx_libs)"

    # We still need the core lib deps for a static build though
    if is_static; then
        link_deps="${libdir}/libwx_msw-2.6-i386-mingw32msvc.a" 
        wx_libs="$wx_libs $link_deps $ldlibs_core -lrpcrt4 -loleaut32 -lole32 -luuid -lwinspool -lwinmm -lshell32 -lcomctl32 -lcomdlg32 -lctl3d32 -ladvapi32 -lwsock32 -lgdi32"
    else
        wx_libs="$wx_libs -lwx_msw-2.6-i386-mingw32msvc"
    fi
    ...
    using_gui=no
    for i in $wx_libs ; do
        if match_field "$i" adv core ; then
            _guildflags=" -Wl,--subsystem,windows -mwindows"
            using_gui=yes
            break
        fi
        match_field "$i" net base || using_gui=yes
    done
...
if is_installed; then
    _include_cppflags="-I${includedir}/wx-2.6"
else
    _include_cppflags="-I${includedir} -I${prefix}/contrib/include"
fi

_cppflags=$(echo "-I${libdir}/wx/include/i386-mingw32msvc-msw-ansi-release-static-2.6" $_include_cppflags "" " -D__WXMSW__" "" $_gui_cppflags "" "" "-DNO_GCC_PRAGMA")  

# now without further ado, we can answer these too.
[ -z "$output_option_cppflags" ] || echo $_cppflags
[ -z "$output_option_cflags"   ] || echo $_cppflags ""
[ -z "$output_option_cxxflags" ] || echo $_cppflags "" " -fno-rtti -fno-exceptions"
